...
public class AuthorizationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        boolean isAuthenticated = request.getAttribute("IS_AUTHENTICATED") != null;
        if (isAuthenticated) {
            HttpSession session = httpRequest.getSession();
            AccountDTO account = (AccountDTO) session.getAttribute("ACCOUNT");
            if (account.getRole() == Role.ADMIN) {
                list = (List<String>) httpRequest.getServletContext().getAttribute(ADMIN);
            } else if (account.getRole() == Role.CUSTOMER) {
                list = (List<String>) httpRequest.getServletContext().getAttribute(CUSTOMER);
            }
